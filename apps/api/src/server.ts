...
const kernel=new IdraKernel(); const memoryEngine=new MemoryEngine(); kernel.registerEngine(new IntentEngine()); kernel.registerEngine(memoryEngine); await kernel.initialize();
app.get("/health",(_req,res)=>res.json({status:"healthy",system:"Intelligence OS",kernel:"Idra",version:"0.2.0",engines:["IntentEngine","MemoryEngine"]}));
